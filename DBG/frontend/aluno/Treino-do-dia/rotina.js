document.addEventListener('DOMContentLoaded', () => {
    ...
    const diarioBar = document.getElementById('prog-diario-barra'); 
    const diarioTexto = document.getElementById('progresso-diario-texto');
    const semanalBar = document.getElementById('prog-semanal-barra');
    ...
    if (finalizarBtn) {
        finalizarBtn.addEventListener('click', () => {
            ...
            const checkboxesDoDia = diaAtivoEl.querySelectorAll('.exercicio-check');
            const totalExercicios = checkboxesDoDia.length;
            const exerciciosConcluidos = diaAtivoEl.querySelectorAll('.exercicio-check:checked').length;

            if (totalExercicios === 0 || exerciciosConcluidos < totalExercicios) {
    ...
    function atualizarProgressoSemanal() {
        ...
        const totalDias = Object.keys(diasConcluidos).length;
        const porcentagem = (diasFinalizados / totalDias) * 100;

        if (semanalBar) {
            semanalBar.value = porcentagem;
        }
        if (semanalTexto) {
